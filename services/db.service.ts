...
class DatabaseService {
  ...
  async getAttendanceLogs(): Promise<AttendanceSession[]> {
    ...
    if (error) throw new Error(formatError(error));
    return (data || []).map(s => ({
      id: s.id, studentId: s.student_id, sessionDate: s.session_date, checkInTime: s.check_in_time, checkOutTime: s.check_out_time, checkoutMode: s.checkout_mode, checked_in_by: s.checked_in_by, checked_out_by: s.checked_out_by, status: s.status, createdAt: s.created_at
    }));
  }
  ...
  async addSession(data: Omit<AttendanceSession, 'id' | 'createdAt'>) {
    const { data: result, error } = await supabase.from('attendance_sessions').insert([{
      student_id: data.studentId, session_date: data.sessionDate, check_in_time: data.checkInTime, checked_in_by: data.checkedInBy, status: data.status
    }]).select().single();
    if (error) throw new Error(formatError(error));
    return result;
  }
  ...
  async getLogs(): Promise<AuditLog[]> {
    ...
    if (error) throw new Error(formatError(error));
    return (data || []).map(l => ({ id: l.id, eventType: l.event_type, actor: l.actor, entityId: l.entity_id, payload: l.payload, createdAt: l.created_at }));
  }

  async getSettings(): Promise<AppSettings> {
    const { data, error } = await supabase.from('app_settings').select('*').single();
    if (error) return { id: 'default', matchThreshold: 0.78, autoCheckoutTime: '13:00', allowDuplicatePoints: false };
    return { id: data.id, matchThreshold: data.match_threshold, autoCheckoutTime: data.auto_checkout_time, allowDuplicatePoints: data.allow_duplicate_points };
  }
